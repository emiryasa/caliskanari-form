      ...
      {Object.entries(sections).map(
        ([sectionName, sectionData]) =>
          typeof sectionData === "object" && (
            <div
              key={sectionName}
              className="w-full sm:w-1/2 md:w-1/3 bg-blue-200 lg:w-1/3 xl:w-1/3 p-4"
            >
...
const SectionInputs = React.memo(
  ({ sectionName, sectionData, handleTextChange, handleColorChange }) => (
    <div className="flex flex-col">
      {sectionData.map((section, index) => (
        <div key={index} className="mb-4 flex flex-col">
          <textarea
            className="border border-black text-black p-2 h-[85px]"
            style={{ resize: "none" }}
            name={`text-${sectionName}-${index}`}
            value={section.text || ""}
            onChange={(e) =>
              handleTextChange(sectionName, e.target.value, index)
            }
            rows={10}
          />
          <input
            ...
            type="color"
            onChange={(e) =>
              handleColorChange(sectionName, e.target.value, index)
            }
          />
        </div>
      ))}
